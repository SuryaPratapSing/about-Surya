...
const Nav = () => {
  ...
  return (
    <div className={`w-full h-16 md:h-24  bg-[#4731d4] ${location.pathname === '/work' ? 'md:static bg-gray-100' : 'md:fixed md:bg-transparent'} top-0 flex justify-between items-end px-5 lg:px-24 z-50 relative`}>
      <div className='h-fit relative'>
        ...
        <div className={showLogo ? 'font-extrabold text-xl h-3 w-1 bg-[#ccf382] absolute -bottom-1 left-[0.6rem]' : 'bg-[#f06c52] font-extrabold text-xl h-3 w-1 absolute -bottom-1 left-[0.6rem]'}>
        </div>
        <div className={showLogo ? 'w-2 h-2 rounded-full bg-[#ccf382] left-[0.46rem] absolute' : 'bg-[#f06c52] w-2 h-2 rounded-full left-[0.46rem] absolute'}>
        </div>
      </div>
          ...
          <div className={`absolute -top-14 -right-14 transform origin-top w-fit h-auto bg-gray-50 rounded shadow-md transition-all duration-300 pl-20 pr-14 py-10 z-40 text-[#4731d4] animate-dropdown ${animationClass}`}>
            ...
            <li className="transition-colors duration-300 relative group z-20 flex items-center cursor-pointer w-fit">
                ...
                {location.pathname === '/' ? null : (
                  <NavLink onClick={toggleDropdown} to={'/'}>Home</NavLink>
                )}
              </li>
              ...
              <li className="transition-colors duration-300 relative group z-20 flex items-center cursor-pointer w-fit">
                <span className="group-hover:bg-blue-400/30 w-5 h-5 -right-3 absolute transition-all duration-300"></span>
                {/* Conditional rendering of NavLink for "Home" */}
                {location.pathname === '/self' ? null : (
                  <NavLink onClick={toggleDropdown} to={'/self'}>My Shelf</NavLink>
                )}
              </li>
